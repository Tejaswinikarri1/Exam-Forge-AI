...
import { ChevronRight, ChevronLeft, Cpu, Upload, FilePlus, Check, AlertTriangle, BookOpen, Download } from 'lucide-react';
import { T, Spin, Card, Btn, Badge, Field, Sel } from '../components/UI';
import { ALL_SUBJECTS, generateMockTest } from '../utils/questions';
import { exportExamToPDF } from '../utils/pdfExport';
...
const CreateExam = ({ onNav, onStartTest }) => {
  ...
  const runAI = async () => {
    try {
      ...
      const examRes = await api.post('/exams', {
        title: cfg.title || (cfg.subject + ' ' + cfg.difficulty + ' Test'),
        subject: cfg.subject,
        topic: cfg.topic,
        difficulty: cfg.difficulty,
        questions: questionCount,
        time: parseInt(cfg.time) || 45,
        type: cfg.type
      });
      const createdExam = examRes.data.exam;
      setExamId(createdExam._id);

      // Step 2-5: Generate with AI
      setAiStep(2);
      await new Promise(r => setTimeout(r, 600));
      setAiStep(3);
      await new Promise(r => setTimeout(r, 600));

      const genRes = await api.post(`/exams/${createdExam._id}/generate`);
      ...
      const generatedQuestions = genRes.data.questions.map((q, i) => ({
        q: q.question,
        opts: q.options,
        ans: q.options ? q.options.indexOf(q.correctAnswer) : null,
        exp: q.explanation,
        source: q.source
      }));
      ...
      setGenerated({
        ...
        time: parseInt(cfg.time) || 45,
        aiModel: genRes.data.model,
        generationTime: genRes.data.generationTime
      });
  ...
  const doPDF = async () => {
    try {
      if (!pdfFile) {
        setPdfErr('Please upload a PDF file first.');
        return;
      }
      ...
      //  AUTO CREATE EXAM (FIX)
      if (!id) {
        const examRes = await api.post('/exams', {
          title: cfg.title || (cfg.subject + ' PDF Test'),
          subject: cfg.subject,
          topic: cfg.topic,
          difficulty: cfg.difficulty,
          questions: questionCount,
      ...
      const pdfRes = await api.post(`/exams/${id}/pdf`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });

      const generatedQuestions = pdfRes.data.questions.map((q, i) => ({
        q: q.question,
        opts: q.options,
        ans: q.options ? q.options.indexOf(q.correctAnswer) : null,
        exp: q.explanation,
      ...
      setGenerated({
        title: 'PDF Extract — ' + cfg.subject,
        subject: cfg.subject,
        difficulty: cfg.difficulty,
        type: cfg.type,
        questions: generatedQuestions,
        time: parseInt(cfg.time) || 45,
        aiModel: pdfRes.data.model + ' (PDF)',
        generationTime: pdfRes.data.generationTime
      });
      ...
      setPdfErr(error.response?.data?.message || 'Failed to process PDF.');
      setPdfParsing(false);
    }
  };
  ...
  const saveExam = async () => {
    try {
      ...
      // Exam already saved with generate call
      setSaved(true);
    } catch (e) { console.error(e); }
  };

  const startNow = () => { onStartTest({ ...generated, title: cfg.title || generated.title, qData: generated.questions, examId }); onNav('take-test'); };
  ...
  return (
    <div className='fu'>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginBottom: 24 }}>
        {['Configure', 'Generate', 'Review & Launch'].map((s, i) => {
          const n = i + 1, act = step === n, done = step > n; return (
      ...
      {step === 2 && (
        <div className='fi'>
          <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
            {[{ id: 'ai', label: 'AI Generate', I: Cpu }, { id: 'pdf', label: 'Upload PDF', I: Upload }, 
            // { id: 'manual', label: 'Manual', I: FilePlus }
          ].map(m => (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                ...
                {generated.aiModel && <Badge color={T.warn}>AI: {generated.aiModel}</Badge>}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              <Btn variant='outline' onClick={() => { setGenerated(null); setStep(2); }}>Regenerate</Btn>
              <Btn variant='outline' onClick={() => exportExamToPDF(generated, generated.questions)}><Download size={14} />Export PDF</Btn>
            ...
            {generated.questions.map((q, i) => (
              <div key={i} style={{ padding: '13px 0', borderBottom: `1px solid ${T.border}33` }}>
                <div style={{ display: 'flex', gap: 10, marginBottom: q.opts ? 10 : 4 }}>
                  ...
                  <span style={{ fontSize: 13, lineHeight: 1.55, fontWeight: 500 }}>{q.question || q.q}</span>
                </div>
